...
from .lenses import LensError, apply_lenses
...
class Parser:
    ...
    def parse(self):
        while not self.eof():
            ...
            if ln.indent != 0:
                raise FACETError(
                    "F002", "Top-level content must start at column 1", ln.no, ln.indent + 1
                )
    ...
    def _parse_facet(self):
        ...
        if not m:
            raise FACETError("F001", "Malformed facet header", header.no, 1)
        name = m.group(1)
        attrs_src = m.group(3)
        attrs = OrderedDict()
        if attrs_src:
        ...
        while not self.eof():
            ...
            if re.match(r"-\s", ln.text):
                if out:  # If we already have keyed items, anonymous items are not allowed
                    raise FACETError(
                        "F101",
                        "Cannot mix anonymous list items with keyed values",
                        ln.no,
                        ln.indent + 1,
                    )
                ...
                if lenses:
                    try:
                        val = apply_lenses(val, lenses)
                    except LensError as e:
                        raise FACETError("F102", str(e), cur.no, cur.indent + 1)
            ...
            if (
                has_anonymous_items
            ):  # If we already have anonymous items, keyed items are not allowed
                raise FACETError(
                    "F101",
                    "Cannot mix anonymous list items with keyed values",
                    ln.no,
            ...
            if val_src is None:
                ...
                if lenses:
                    try:
                        ...
                        raise FACETError("F102", str(e), ln.no, ln.indent + 1)
                if anchor:
    ...
    def _parse_list(self, expected_indent):
        ...
        while not self.eof():
            ...
            if ln.indent > expected_indent:
                raise FACETError(
                    "F002", "Unexpected deeper indentation inside list", ln.no, ln.indent + 1
                )
            cur = self.next()
            m = re.match(r"-\s(.*)$", cur.text)
            if not m:
    ...
    def _parse_kv_head(self, line: Line):
        ...
        if not m:
            raise FACETError("F001", "Malformed key-value", line.no, line.indent + 1)
        key = m.group(1)
        anchor = m.group(2)
        val_src = m.group(3).strip()
    ...
    def _parse_lens(self, pipe: str):
        ...
        if not m:
            raise FACETError("F102", f"Malformed lens: {pipe}")
        name = m.group(1)
        args_src = m.group(3)
        args = []
        if args_src and args_src.strip():
            for p in self._split_commas(args_src):
                ...
                elif (p.startswith('"') and p.endswith('"')) or (
    ...
    def _read_fence(self, first: str):
        ...
        if not m:
            raise FACETError("F001", "Fenced block must start with ``` on its own line")
        content_lines = []
        while not self.eof():
            ln = self.next()
    ...
    def _split_kv(self, part: str, eq=":"):
        if eq == ":":
            ...
            m = re.match(rf"\s*({IDENT})\s*=\s*(.+)\s*$", part)
        if not m:
            raise FACETError("F001", f"Malformed pair: {part}")
